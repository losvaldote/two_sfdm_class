...
int spectra_indices(
  ...
  class_define_index(psp->index_tr_delta_sfdm_1,ppt->has_source_delta_sfdm_1,index_tr,1);
  /*Scalar Field Dark Matter 2*/
  class_define_index(psp->index_tr_delta_sfdm_2,ppt->has_source_delta_sfdm_2,index_tr,1);
  ...
  /*Scalar Field Dark Matter 1*/
  class_define_index(psp->index_tr_theta_sfdm_1,ppt->has_source_theta_sfdm_1,index_tr,1);
  /*Scalar Field Dark Matter 2*/
  class_define_index(psp->index_tr_theta_sfdm_2,ppt->has_source_theta_sfdm_2,index_tr,1);
      ...
      for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
        ...
        /*Scalar Field Dark Matter 1*/
          if (pba->has_sfdm_1 == _TRUE_) {
              ...
              rho_i = pvecback_sp_long[pba->index_bg_rho_sfdm_1];

              if (ppt->has_source_delta_sfdm_1 == _TRUE_) {

                  delta_i = ppt->sources[index_md]
                  [index_ic * ppt->tp_size[index_md] + ppt->index_tp_delta_sfdm_1]
                  [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

                  psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_delta_sfdm_1] = delta_i;
              ...
              if (ppt->has_source_theta_sfdm_1 == _TRUE_) {
                  ...
                  theta_i = ppt->sources[index_md]
                  [index_ic * ppt->tp_size[index_md] + ppt->index_tp_theta_sfdm_1]
                  [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

                  psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_theta_sfdm_1] = theta_i;

                  rho_plus_p_theta_tot += (rho_i + pvecback_sp_long[pba->index_bg_p_sfdm_1]) * theta_i;

              }

              rho_plus_p_tot += (rho_i + pvecback_sp_long[pba->index_bg_p_sfdm_1]);
          ...
          /*Scalar Field Dark Matter 2*/
            if (pba->has_sfdm_2 == _TRUE_) {
                ...
                rho_i = pvecback_sp_long[pba->index_bg_rho_sfdm_2];

                if (ppt->has_source_delta_sfdm_2 == _TRUE_) {

                    delta_i = ppt->sources[index_md]
                    [index_ic * ppt->tp_size[index_md] + ppt->index_tp_delta_sfdm_2]
                    [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

                    psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_delta_sfdm_2] = delta_i;
                ...
                if (ppt->has_source_theta_sfdm_2 == _TRUE_) {
                    ...
                    theta_i = ppt->sources[index_md]
                    [index_ic * ppt->tp_size[index_md] + ppt->index_tp_theta_sfdm_2]
                    [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

                    psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_theta_sfdm_2] = theta_i;

                    rho_plus_p_theta_tot += (rho_i + pvecback_sp_long[pba->index_bg_p_sfdm_2]) * theta_i;

                }

                rho_plus_p_tot += (rho_i + pvecback_sp_long[pba->index_bg_p_sfdm_2]);
...
int spectra_output_tk_titles(struct background *pba,
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      /*Scalar Field Dark Matter 2*/
      class_store_columntitle(titles,"d_sfdm_2",pba->has_sfdm_2);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      /*Scalar Field Dark Matter 2*/
      class_store_columntitle(titles,"t_sfdm_2",pba->has_sfdm_2);
    ...
    /*Scalar Field Dark Matter 1*/
    class_store_columntitle(titles,"-T_sfdm_1/k2",pba->has_sfdm_1);
    /*Scalar Field Dark Matter 2*/
    class_store_columntitle(titles,"-T_sfdm_2/k2",pba->has_sfdm_2);
      ...
      for (index_k=0; index_k<psp->ln_k_size; index_k++) {
        ...
        if (output_format == class_format) {
          ...
          if (ppt->has_density_transfers == _TRUE_) {
            ...
            /*Scalar Field Dark Matter 1*/
            class_store_double(dataptr,tk[psp->index_tr_delta_sfdm_1],ppt->has_source_delta_sfdm_1,storeidx);
            /*Scalar Field Dark Matter 2*/
            class_store_double(dataptr,tk[psp->index_tr_delta_sfdm_2],ppt->has_source_delta_sfdm_2,storeidx);
          ...
          if (ppt->has_velocity_transfers == _TRUE_) {
            ...
            /*Scalar Field Dark Matter 1*/
            class_store_double(dataptr,tk[psp->index_tr_theta_sfdm_1],ppt->has_source_theta_sfdm_1,storeidx);
            /*Scalar Field Dark Matter 2*/
            class_store_double(dataptr,tk[psp->index_tr_theta_sfdm_2],ppt->has_source_theta_sfdm_2,storeidx);
          ...
          /*Scalar Field Dark Matter 1*/
          class_store_double_or_default(dataptr,-tk[psp->index_tr_delta_sfdm_1]/k2,ppt->has_source_delta_sfdm_1,storeidx,0.0);
          /*Scalar Field Dark Matter 2*/
          class_store_double_or_default(dataptr,-tk[psp->index_tr_delta_sfdm_2]/k2,ppt->has_source_delta_sfdm_2,storeidx,0.0);
