...
int background_functions(
  ...
  /* Scalar field dark matter 1. It should be the last density component to be computed! */
  ...
  /* Scalar field dark matter. It should be the last density component to be computed! */
...
int background_solve(
  ...
  //if (pba->background_verbose > 0) {
    ...
    if (pba->has_sfdm_1 == _TRUE_){
        ...
        /*cosh potential*/ //Revisar
        /*if(pba->sfdm_parameters_1[1] < 0.){
          ini_displacement_1 = exp(pba->y1_ini_sfdm_1-0.5*pba->alpha_ini_sfdm_1)*
                              pow(1.-pba->sfdm_parameters_1[1]*exp(pba->alpha_ini_sfdm_1-2.*pba->y1_ini_sfdm_1)
                              *(1.+cos_sfdm(pba, pba->theta_ini_sfdm_1)),0.5)/pow(2.*pba->sfdm_parameters_1[1],0.5);
         printf("Initial field misalignment:\n");
         printf("-> sfdm_1_ini/f = %1.2e [Rad], %1.2e [Deg]\n", 2.*atan(ini_displacement_1),
                2.*atan(ini_displacement_1)*180./_PI_);
        }*/
      }
      /*Scalar Field Dark Matter 2*/
      if (pba->has_sfdm_2 == _TRUE_){
        ...
        /*cosh potential*/ //Revisar
        /*if(pba->sfdm_parameters_2[1] < 0.){
          ini_displacement_2 = exp(pba->y1_ini_sfdm_2-0.5*pba->alpha_ini_sfdm_2)*
                              pow(1.-pba->sfdm_parameters_2[1]*exp(pba->alpha_ini_sfdm_2-2.*pba->y1_ini_sfdm_2)
                              *(1.+cos_sfdm(pba, pba->theta_ini_sfdm_2)),0.5)/pow(2.*pba->sfdm_parameters_2[1],0.5);
         printf("Initial field misalignment:\n");
         printf("-> sfdm_2_ini/f = %1.2e [Rad], %1.2e [Deg]\n", 2.*atan(ini_displacement_2),
                2.*atan(ini_displacement_2)*180./_PI_);
        }*/
        }
////////////////////////////////////////////////////////////////
    if(pba->has_lambda == _TRUE_)
  ...
  if (pba->has_sfdm_1 == _TRUE_){
    ...
    if (pba->sfdm_parameters_1[1] >= 0.){
      ...
      dy[pba->index_bi_theta_sfdm_1] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_1])+y[pba->index_bi_y1_sfdm_1]);
